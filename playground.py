...
/** Strip shared `agent__` prefix for display when all ids containing `__` agree (other ids stay full-length). */
let _cachedModelDisplayPrefix = null
function sharedModelDisplayPrefix(){
  if(_cachedModelDisplayPrefix !== null) return _cachedModelDisplayPrefix
  const keys = Object.keys(RAW.models).filter(k=>k.includes('__'))
  if(!keys.length){ _cachedModelDisplayPrefix = ''; return '' }
  let first = null
  for(const k of keys){
    const i = k.indexOf('__')
    const pre = k.slice(0, i + 2)
    if(first === null) first = pre
    else if(pre !== first){ _cachedModelDisplayPrefix = ''; return '' }
  }
  _cachedModelDisplayPrefix = first
  return first
}
...
function renderFieldHeatmap(){
  ...
  for(const m of models){
    html += `<tr><td style="font-size:12px;font-weight:600;white-space:nowrap">${modelDisplayName(m)} <span style="font-weight:400;color:var(--muted);font-size:10px">${providerLabel(m)}</span></td>`
...
function renderDocHeatmap(){
  ...
  for(const m of models) html += `<th style="writing-mode:vertical-lr;transform:rotate(180deg);height:80px;font-size:10px">${modelDisplayName(m)}</th>`
  html += '</tr></thead><tbody>'
...
function renderRecommendations(){
  ...
  if(scores.length){
    ...
    html += `<div class="insight insight-tip" style="margin-top:16px">
      <strong>Recommended:</strong> <strong>${modelDisplayName(top.m)}</strong> (${costTierBadge(top.m)}) achieves ${pct(top.acc)} on the selected fields.
      ${scores[1] && scores[1].m!==top.m?`Runner-up: <strong>${modelDisplayName(scores[1].m)}</strong> at ${pct(scores[1].acc)}.`:''}
    </div>`
    if(bestFree && bestFree.m!==top.m){
      html += `<div class="insight" style="margin-top:8px"><strong>Best free option:</strong> <strong>${modelDisplayName(bestFree.m)}</strong> at ${pct(bestFree.acc)}.</div>`
    }
  }
...
function renderImprovements(){
  ...
  const failedNames = allModels().filter(m=>!isActive(m)).map(modelDisplayName).join(', ')
