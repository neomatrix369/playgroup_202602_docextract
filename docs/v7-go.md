...
|---------|----------------|------------------------|-----------------|----------------------------------------|
| **OpenRouter** | `config_models_openrouter.py` | 39 | Manual per-model | From API token usage + config |
| **Doubleword** | `config_models_doubleword.py` | 35 extraction models | **Manual** (auto-sync disabled via `SKIP_DOUBLEWORD_SYNC=1`; identifiers corrected from API) | Batch metadata + tokens + config |
